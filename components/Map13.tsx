  ...
  const apiProjects: Project[] = [
    {
      ...
      images: ["/img4.jpg", "/img5.JPG", "/img4.jpg"], // ✅ gallery
    },
    {
      id: "2",
      beneficiaries: 3500,
                  ...
                  <div className="relative mt-2 group">
                    ...
                    {proj.images.length > 1 && (
                      <span className="absolute bottom-2 right-2 bg-black/60 text-white text-xs px-2 py-1 rounded-md">
                        +{proj.images.length - 1} صور
                      </span>
                    )}
                    {/* Hover Overlay with Other Images */}
                    {proj.images.length > 1 && (
                      <div
                        className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 
                 transition-opacity duration-300 flex items-center justify-center gap-2 p-2"
                      >
                        {proj.images.slice(1, 4).map((img, idx) => (
                          <Image
                            key={idx}
                            src={img}
                            width={80}
                            height={80}
                            alt={`Preview ${idx + 2}`}
                            className="w-20 h-16 object-cover rounded-md border border-white/20 
